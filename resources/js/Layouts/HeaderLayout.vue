<template>
  <div class="h-full"> 
      ...
      <div class="flex flex-col h-full">
        <div class="flex justify-between sm:h-full h-16">
          <div class="flex flex-row items-center justify-center ml-6">
            <button @click="nav" class="hover:bg-white p-2 rounded focus:outline-none w-10 flex justify-center" aria-label="display menu">
          ...
          <form class="flex items-center justify-center" @submit.prevent="search" v-if="this.windowWidth > 640">
            <input class="rounded rounded-r-none w-96 h-8" type="text" id="search" aria-label="Search for videos">
          ...
          <div class="flex items-center text-white font-medium mr-6">
              <div class="text-center">Have questions? Email 
                <a class="hover:bg-white hover:text-black underline text-color" href="mailto:support@vodunion.com">support@vodunion.com</a>
              </div>
          </div>
        </div>
        <form class="shadow-md flex items-center justify-center fixed top-16 w-full m-b-8" @submit.prevent="search" v-if="this.windowWidth < 640">
          <input class="rounded rounded-r-none w-full h-8" type="text" id="search" aria-label="Search for videos">
    ...
    <main class="overflow-auto" v-bind:class="{ 'pt-20': (this.windowWidth > 640), 'pt-28': (this,windowWidth < 640)}">
